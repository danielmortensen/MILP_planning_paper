...
An additional contribution this work offers is the expansion to joint optimization of both night and day charging in a single optimization problem. Day and night operations differ in two aspects: number of chargers and bus availability. During the day, the buses can charge only at the charge station. The number of chargers in the station are limited, causing contention between buses. At night, each bus docks in a holding stall with one charger per stall, eliminating charger contention. Furthermore, nighttime charging is slow compared to daytime charging.  Our model uses different rates for day and night charging.
