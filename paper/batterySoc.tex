...
where $d_{i,k}$ is the SOC as seen in Fig.~\ref{fig:dSocDiagram}. The set of all $d_{i,k}$ can be organized as the vector $\mathbf{d}$ from \eqref{eqn:y}.
...
The constraints from \eqref{eqn:dDelta} can be expressed in linear standard form as 
...
\begin{equation}\label{eqn:initialGConstr}
	...
	\end{aligned}
\end{equation}
The conditions given in \eqref{eqn:initialGConstr} can be rewritten as 
...
\end{aligned}
\end{equation}
where $M$ is the battery capacity. The results of \eqref{eqn:bigM2} obtain a switching effect.  When $x_{i,k,l} = 1$, \eqref{eqn:bigM2} becomes 
\begin{equation}
	\begin{aligned}
		& \begin{rcases}
			g_{i,k,l} \le d_{i,k}(\bar{a}_l - 1) - \bar{b}_lM & \\
	...
	\end{aligned}
\end{equation}
The active constraints imply equality for $g_{i,k,l}$ = $(\bar{a}_l - 1)d_{i,k} - \bar{b_l}M$.  The inactive constraints imply that $g_{i,k,l}$ is greater than zero and less than the battery capacity, which are trivially satisfied. When $x_{i,k,l} = 0$, \eqref{eqn:bigM2} becomes
\begin{equation}
	\begin{aligned}
		& \begin{rcases}
...
\begin{equation}\label{eqn:socMat}
	...
	\end{bmatrix}.
\end{equation}
Equation (\eqref{eqn:socMat}) can be expanded to include constraints for all $g_{i,k,l}$.  Because each value for $g_{i,k,l}$, $d_{i,k}$, and $x_{i,k,l}$ is an element of $\mathbf{y}$, the constraints from \eqref{eqn:socMat} can be written as 
...
\begin{equation}\label{eqn:tau}
	...
	\end{aligned}
\end{equation}
Equation (\eqref{eqn:tau}) can be expanded to a matrix $D_\tau$, where each $d_{i,k}$ contains a corresponding constraint row such that
...
as given in \eqref{eqn:totalG}.  Gains are computed using a
switching constraint which sets them to zero when not charging,
otherwise they follow the CCCV model as set forth in 
\eqref{eqn:CCCVFinal}. Initial SOC values are handled with the equality
constraint given in \eqref{eqn:dInitialFinal} and the SOC is
constrained to remain above the threshold $\tau$ in 
\eqref{eqn:tauFinal}. All constraints for $d$ can be concatenated such
that
\begin{equation}
	\begin{bmatrix}
	D_0 \\
